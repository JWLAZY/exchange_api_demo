...
module.exports = {
     ...
     * 订单代币转账
     */
    transTokenForOrder: (formid,toid,count,tokenaddress,callback) => {
        findOneUser(formid,(error, fuser) => {
            findOneUser(toid, async (error, tuser)=>{
                let fromAddress = fuser.ethaddress; // 转币方
                let toAddress = tuser.ethaddress; // 接收方
                let fromKey = fuser.ethkey; // 转币方私钥
                let contractAddress = tokenaddress; // 代币地址
                // 代币转账 相比较 ether 转账而言,就是 不传value 传data(data:智能合约方法的16机制字符串)
                // to : 代币交易接收方是代币地址
                // 1. 拿到智能合约实例
